//----This is the lamp with wrong location------//
...
let lamp = {
    ...
    location:'DaYanBuilding.2rdFloor.201Room.ceiling'
  };
...
mdns.on('query', function(query){
    //---------PTR---------//
      ...
      if(query.questions[0].name === '_light._udp.local' && query.questions[0].type === 'PTR'){
        ...
        let sqe=(txt.decode(txt.encode(query.additionals[0]))).data;
        let sqe2=(txt.decode(txt.encode(query.additionals[1]))).data;
        let sqe3=(txt.decode(txt.encode(query.additionals[1]))).data;
        //console.log(sqe2)
        let sre=lamp.location
        console.log("搜尋地點1:"+sqe);
        console.log("搜尋地點2:"+sqe2);
        console.log("搜尋地點2:"+sqe3);
        console.log("燈泡實際地點:"+sre);
        matcher.add(sqe);
        matcher.add(sqe2);
        matcher.add(sqe3);
        if(matcher.test(lamp.location)===true){
